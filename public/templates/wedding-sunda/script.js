/* ============================================================
   TEMPLATE: SUNDA KEMBANG KOL — ULTIMATE SCRIPT
...
const D = {
    // Identitas
    groom:          param('groom',          'Asep'),
    bride:          param('bride',          'Neneng'),
    groomFull:      param('groomFull',      'Asep Sunandar, S.Kom.'),
    brideFull:      param('brideFull',      'Neneng Kurnia, S.Pd.'),
    groomParent:    param('groomParent',    'Putra ka-2 ti Bapak H. Nana & Ibu Hj. Siti'),
    brideParent:    param('brideParent',    'Putri ka-1 ti Bapak Dadang & Ibu Enny'),
    groomIg:        param('groomIg',        '@asep_s'),
    brideIg:        param('brideIg',        '@neneng_k'),
    // Tanggal & Waktu
    dateDisplay:    param('date',           '25 Desember 2026'),
    dateFull:       param('dateFull',       'Minggu, 25 Desember 2026'),
    // Akad
    akadDate:       param('akadDate',       'Minggu, 25 Des 2026'),
    akadTime:       param('akadTime',       '08.00 - 10.00 WIB'),
    akadPlace:      param('akadPlace',      'Masjid Raya Bandung'),
    akadAddress:    param('akadAddress',    'Jl. Dalem Kaum No.14, Bandung'),
    akadISO:        param('akadISO',        '2026-12-25T08:00:00'),
    akadMaps:       param('akadMaps',       'Masjid+Raya+Bandung'),
    // Resepsi
    resepsiDate:    param('resepsiDate',    'Minggu, 25 Des 2026'),
    resepsiTime:    param('resepsiTime',    '11.00 - Selesai'),
    resepsiPlace:   param('resepsiPlace',   'Gedung Sate Bandung'),
    resepsiAddress: param('resepsiAddress', 'Jl. Diponegoro No.22, Bandung'),
    resepsiISO:     param('resepsiISO',     '2026-12-25T11:00:00'),
    resepsiMaps:    param('resepsiMaps',    'Gedung+Sate+Bandung'),
    // Amplop
    bankName:       param('bankName',       'Bank BCA'),
    bankAcc:        param('bankAcc',        '1234567890'),
    bankHolder:     param('bankHolder',     'Asep Sunandar'),
    // Lain-lain
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-copy').forEach(b => {
        b.addEventListener('click', () => {
            ...
            navigator.clipboard.writeText(num).then(() => toast('✅ Rekening disalin!'));
        });
    });
    ...
    if(btnWa) {
        btnWa.addEventListener('click', () => {
            ...
            if(!name){ toast('⚠️ Mohon isi nama anjeun'); return; }
            const att = document.querySelector('input[name="att"]:checked').value;
            const text = `Sampurasun! Sim kuring ${name} bade ngonfirmasi kahadiran: ${att}\n\nSeratan: ${$('rsvp-msg').value}`;
